...
상담원 PC 에 USB 헤드셋(AB13X USB Audio, 마이크·헤드폰 모두 통신용 기본)이 붙어 실측을 돌렸다.
...
음향 결합이 있으면 되돌아온 소리가 다시 마이크로 들어가 왕복을 한 번 더 돈다.
...
배수 자리에 봉우리가 없다 = **재순환 에코 없음.** 헤드셋 조건 합격선("상대 레그에 상담원 음성이 들리지 않는다")을 만족한다.

**스피커폰 조건은 여전히 미검증이다.** 이 PC 에는 내장 스피커/마이크가 없어 조건 자체를 만들 수 없다.
