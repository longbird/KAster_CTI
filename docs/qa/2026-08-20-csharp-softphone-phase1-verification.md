...
## 12. 상담원 기능 보강 2차와 실기기 전환 (2026-08-21)

사용자가 **실기기 모드를 기본으로** 요구하면서, 그동안 소프트폰 경로가 가려 주던 결함이 한꺼번에 드러났다.
소프트폰은 끊기·음소거가 로컬로도 동작해 서버 실패가 티가 나지 않았지만, 실기기는 서버가 유일한 경로다.

### 12-1. 추가한 기능

| 기능 | 비고 |
|---|---|
| 통화 메모 | 통화 종료 시 저장. 거부되면 화면에 남겨 다시 쓸 수 있게 한다 |
| 로그아웃 | 서버가 죽어도 진행. 통화 중에는 막는다 |
| 서버 이벤트 반영 | `agent.status.changed`(관리자 강제 이석), `screenpop.customer` |
| **소프트폰 / 실기기 모드** | 로그인 시 선택. **기본은 실기기** |
| 실기기 등록 확인 + 등록 정보 안내 | 미등록이면 전화기에 넣을 값을 보여 준다. 비밀번호는 가림 |
| 설정 화면 | 서버 주소(저장 전 검증) + 오디오 장치 3종 |
| 당겨받기 | 큐 대기 통화 목록. **목록 / 타일** 보기 선택 |
| 전화번호 표시 형식 | `010-3462-3453`. 내선·119 는 나누지 않는다 |

### 12-2. 이 과정에서 찾아 고친 결함

| # | 결함 | 증상 | 조치 |
|---|---|---|---|
| 1 | AOR 에 죽은 Contact 가 쌓임 | 재시작 몇 번이면 등록이 403 으로 잠김 | `remove_existing=yes` |
| 2 | NAT 구멍이 등록 사이에 닫힘 | INVITE 유실, 전화가 안 옴 | `qualify_frequency=30` + **클라이언트가 OPTIONS 에 200 응답** |
| 3 | 엔드포인트가 WebRTC 전용 | 데스크폰이 울리다 488(cause 58)로 거절 | `webrtc`/`dtls`/`avpf`/`ice`/`rtcp_mux` 제거 → 평문 RTP |
| 4 | **통화 leg 미기록** | 마이크 끄기·끊기·전환·홀드·당겨받기 전부 "채널을 찾을 수 없습니다" | 세션 엔진이 AMI 이벤트마다 leg 기록 |
| 5 | 발신 자동응답 경쟁 | INVITE 가 ack 보다 1ms 빨라 자동응답 실패 → 무응답 종료 | 요청 **전에** 창을 연다 |
| 6 | 이석이 큐를 막지 않음 | 이석인데 전화기가 울림 | 상태 변경이 `QueuePause` 를 함께 보냄 |
| 7 | 큐 대기 통화가 자기 전화로 뜸 | 그 큐의 모든 상담원 화면이 같은 통화로 덮임 | 배정 안 된 통화는 **우리가 방금 걸었을 때만** 채택 |

3·4·6 은 실기기 모드가 없었으면 계속 가려져 있었을 결함이다.
...
```
Newstate      Ringing
HangupRequest Cause: 58
Hangup        Cause: 58 | Cause-txt: "Bearer capability not available"
```

**전환 후** — 서버 로그에 `RTCPReceived`/`RTCPSent` 가 통화 내내 이어지고 `BridgeLeave` 로 종료.
외부 발신(`010-3462-3453`)이 트렁크를 통해 실제로 연결됐다.

**leg 기록**
```
direction: outbound   callLegs: 2건
  - inbound PJSIP/trunk-070-5234-6380-00000026
  - agent   PJSIP/1001-00000025
```

**이석 → 큐 일시정지**
```
홍길동 (PJSIP/1001) (paused:BREAK was 22 secs ago)
```

**당겨받기** — 이석 상태로 큐에 5건을 넣자 목록에 3건 + `외 2건`, 타일로 바꾸면 5건 전부 표시.

### 12-4. 남긴 것

- **`primaryAgentId` 가 발신 세션에서 비어 있다.** 지금은 leg 로 소유권 검사가 통과하지만
  통화 이력·상담원별 통계가 이 값을 쓴다.
- **WebRTC 를 버렸다.** 브라우저 통화가 필요한 현장이 생기면 그 내선만 별도 엔드포인트가 필요하다
  (공백 분석 A안 — 상담원별 단말 종류).
- `ani` 가 `<unknown>` 으로 오는 경우가 있다. 시험용 Local 채널의 한계인지 실제 트렁크에서도
  생기는지는 외부 수신으로 확인해야 한다.
