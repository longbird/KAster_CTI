...
- 작은 `compact` 폭의 로그인 카드
- 과한 설명 문구 없이 제목 + 필수 입력만 노출
- 운영툴처럼 빠르게 입력 가능한 밀도의 폼
...
- `콜센터 서버 URL`
  - 서버 설정이 없는 첫 실행 시에만 본문 노출
  - 한 번 저장된 뒤에는 `고급 옵션` 내부로 이동
- `고급 진단` 또는 `수동 handoff`
  - 기본 화면에서는 숨김
  - 디버그 기능 또는 별도 액션으로만 진입
...
- 기본 화면은 일반 로그인이다.
- 화면은 작은 `compact` 카드로 유지한다.
- 기본 상태에서는 `로그인 ID / 내선 번호 / 비밀번호 / 로그인 버튼`만 보이게 한다.
- 서버 설정이 없을 때만 `콜센터 서버 URL`을 본문에 노출하고, 저장 후에는 `고급 옵션`으로 접는다.
...
- 설치 상태, 웹 자동 연결 상태, 버전 정보, 진단 링크도 기본 로그인 화면에서는 숨기고 별도 액션으로만 보여준다.
...
- 데스크톱 로그인 화면을 작은 `compact` 카드로 축소
- 서버 URL 조건부 노출 + 고급 옵션 접기
