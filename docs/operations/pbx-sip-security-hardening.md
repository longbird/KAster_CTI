...
외부에서 `INVITE sip:001...@<PBX>:36070` 형태로 반복 유입되는 SIP 스캔/툴 스푸핑 공격을 운영 PBX가 요금 발생 없이 차단하고, 반복 공격 IP를 자동 격리할 수 있게 준비한다.

## 현재 확인된 상태

- 운영 PBX는 UDP `36070`에서 SIP를 수신한다.
...
| fail2ban jail 템플릿 | `infra/security/pbx-sip-hardening/fail2ban/kaster-pbx-sip.conf.example` | UDP `36070` 반복 실패 IP ban |
...
```bash
cd /home/blueadm/kaster_cti
bash scripts/pbx-sip-security-prepare.sh --sip-port 36070
```
...
```bash
cd /home/blueadm/kaster_cti
sudo bash scripts/pbx-sip-security-prepare.sh --apply --sip-port 36070
```
...
주의: 현재 템플릿은 trust IP 외 UDP `36070`에 대해 분당 20회 초과 패킷을 drop한다. 실제 상담원 전화기가 공인망에서 직접 붙는 구조라면 IP 대역을 먼저 확정해야 한다.
