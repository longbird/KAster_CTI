...
미처리 이벤트가 하나라도 남으면 그 테넌트의 스풀 커서가 **얼어붙는다.** 이는 의도된 동작이다 —
커서는 단일 포인터라 실패한 이벤트를 넘어가면 그 이벤트가 영구히 유실된다.

- 증상: `offlineEventQueueDepth` 가 줄지 않고 `RECOVERING` 이 유지된다
- 해소: Recovery Coordinator 가 재처리를 **전량 성공**하면 자동으로 풀린다
- 계속 안 풀리면 `recoveryAuditLog` 의 `RECOVERY_FINISHED` 에서 실패 원인을 본다.
  실패가 남은 채로 `NORMAL` 로 되돌리지 않는 것이 이 설계의 의도다
