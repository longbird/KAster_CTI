...
제품의 SIP 수신 포트는 **`48950`** 이다 (`tenantSystemSettings.sipRegisterPort` 기본값).
이 문서의 명령과 템플릿은 모두 `48950` 기준이다.

포트를 바꾸면 **PBX 설정 재적용과 방화벽을 반드시 함께** 맞춰야 한다. 한쪽만 바꾸면
방화벽이 엉뚱한 포트를 지키거나, 새 포트가 무방비로 열린다.

## 확인된 상태

- 운영 PBX는 UDP `48950`에서 SIP를 수신한다.
