...
| # | 질문 | 결정 |
|---|---|---|
| 1 | 제안 타임아웃 | **10초**. 넘기면 다음 상담원으로 |
| 2 | 동시 제안 vs 순차 | **순차**. 큐 전략(`leastrecent`)을 그대로 살린다 |
| 3 | 연속 거절 시 자동 이석 | **하지 않는다**. PBX 의 `autopause` 는 끈다 (아래) |
| 4 | 앱이 꺼진 상담원 | **큐에서 제외한다** (사용자 결정) |

4번이 중요한 이유: 앱이 꺼져 있으면 제안을 받을 수단이 없다. 그대로 두면 그 자리로 간
호가 매번 10초를 버리고 다음으로 넘어간다. 발신자에게는 이유 없는 지연으로만 보인다.

### 4번 구현 방식

WS 연결 여부를 큐 pause 로 반영한다. 이미 있는 `AsteriskManagerService.setQueuePaused` 를 쓴다.

- WS 연결 끊김 / 로그아웃 → pause
- WS 연결 → 상담원의 근무 상태를 다시 계산해서 반영

**주의**: 상담원이 스스로 이석해 둔 상태에서 앱을 껐다 켜면, 연결됐다고 무조건 unpause 하면
안 된다. pause 여부는 한 곳에서 계산한다:

```
shouldPause(agent) = 앱 미접속 || pausesQueueAssignment(agent.statusCode)
```

이 함수를 WS 연결/해제와 상태 변경 **양쪽에서** 호출한다. 한쪽만 부르면 두 상태가 어긋나
"앱은 켜져 있는데 전화가 안 오는" 또는 그 반대가 된다.
