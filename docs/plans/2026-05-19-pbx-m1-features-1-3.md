...
> **Completion reconciliation (2026-05-22):** 이 실행계획서는 최신 상위 계획서 `docs/plans/pbx-selected-features-development-plan-20260514.md`와 커밋 이력을 기준으로 완료 상태로 정리했다. 운영 DB `prisma migrate deploy`와 운영 PBX 반영 검증은 코드 완료 판정에 포함하지 않고 별도 운영 절차로 분리한다.
...
- 상위 계획서: `docs/plans/pbx-selected-features-development-plan-20260514.md` (마일스톤 M1, 기능 1~3)
- 도움말 컴포넌트 명세: `docs/plans/2026-05-19-pbx-feature-help.md` (Chunk 1~3 = Task 1~6)
- 내부 경계 참고:
...
도움말 명세서(`docs/plans/2026-05-19-pbx-feature-help.md`)에 7개 도움말 파일의 **완전한 코드**가 이미 작성되어 있다. 본 청크는 그 명세서의 **Task 1~6(Chunk 1~3)** 을 그대로 실행해 컴포넌트 기반을 만든다. 코드를 여기 복제하지 않는다(DRY).
...
`docs/plans/2026-05-19-pbx-feature-help.md`의 Task 1~6을 명세서에 적힌 코드와 커밋 메시지 그대로 수행한다. (Task 7 "6개 화면 적용"과 Chunk 5 "빌드 스크립트"는 본 플랜 범위 밖 — **실행하지 않는다**.)
