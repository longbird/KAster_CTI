...
- 개발/내부 테스트 단계에서는 unsigned 패키지를 허용할 수 있음
- 시범 운영 단계에서는 운영사 내부 CA 또는 고객사 AD CS 기반 내부 코드서명을 권장
- 정식 운영 전부터는 모든 Windows 설치 패키지에 운영사 코드서명을 필수 적용
- 앱은 다운로드 후 해시를 검증해야 함
- 운영체제 수준 코드서명 검증 실패 시 설치를 중단해야 함

### Code Signing Policy

- 현재 프로젝트의 관리형 Windows PC 전제에서는 초기 개발과 제한된 내부 테스트 동안 unsigned 배포가 가능하다.
- 다만 자동업데이트와 운영사 공식 배포본 검증을 위해, 시범 운영부터는 내부 CA 기반 코드서명을 준비하는 편이 좋다.
- 정식 운영 전에는 반드시 운영사 코드서명 체계를 적용해야 하며, 최소 수준은 `운영사 또는 고객사 AD CS 기반 내부 코드서명 + 상담원 PC 루트 인증서 배포`다.
- 이후 외부 신뢰 또는 공개 배포 요구가 생기면 Trusted Signing 또는 공인 EV 기반 서명으로 확장한다.
