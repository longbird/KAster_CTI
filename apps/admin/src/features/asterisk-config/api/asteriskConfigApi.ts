...
export const getTrunkGroups = () =>
  axios.get<{ data: AsteriskTrunkGroup[] }>(`${base}/trunk-groups`, { headers: headers() }).then(r => r.data.data);
...
export const getSpeedDials = () =>
  axios.get<{ data: AsteriskSpeedDial[] }>(`${base}/speed-dials`, { headers: headers() }).then(r => r.data.data);
